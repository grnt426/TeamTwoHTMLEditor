...
/**
 * Created with IntelliJ IDEA. User: Kocsen Chung Date: 3/17/13 Time: 7:48 PM
 * This is the main frame for the editor.  Will be the class that launches
 * (invokes) only once.
 */
...
public class EditorFrame extends JFrame{
	...
	private JMenu menuFile, menuEdit, menuInsert, menuOptions, menuHTML,
			menuAbout;
	private JMenuItem newMenuItem, openMenuItem, saveMenuItem, saveAsMenuItem,
			closeTabMenuItem, validateMenuItem, quitMenuItem, copyMenuItem,
			pasteMenuItem, tabWidthMenuItem, aboutUsMenuItem, helpMenuItem;
	...
	private void initComponents(){
		...
		URL saveAsMenuItemURL =
				getClass().getResource("/stock_save_template.png");
		URL validateURL = getClass().getResource("/ok.png");
		URL closeTabURL =
				getClass().getResource("/stock_data_delete_table.png");

		if(newMenuItemURL != null){
			newMenuItem.setIcon(new ImageIcon(newMenuItemURL));
		}
		if(openMenuItemURL != null){
			openMenuItem.setIcon(new ImageIcon(openMenuItemURL));
		}
		if(saveMenuItemURL != null){
			if(newMenuItemURL != null){
				saveMenuItem.setIcon(new ImageIcon(saveMenuItemURL));
			}
		}
		if(saveAsMenuItemURL != null){
			saveAsMenuItem.setIcon(new ImageIcon(saveAsMenuItemURL));
		}
		if(validateURL != null){
			validateMenuItem.setIcon(new ImageIcon(validateURL));
		}
		if(closeTabURL != null){
			closeTabMenuItem.setIcon(new ImageIcon(closeTabURL));
		}
		...
		aboutUsMenuItem.addActionListener(new ActionListener(){
			@Override
			public void actionPerformed(ActionEvent e){
				JOptionPane.showMessageDialog(aboutUsMenuItem,
											  "We are the great " +
	...
	private void newMenuItemActionPerformed(){
		new NewFileCommand("File"
						   + Integer.toString(newFileCount)).execute(commandDistributor);

		TabFrame newTabFrame =
				new TabFrame(this); //KEY EVENT 1 : Make new Tab Frame
		tabFrames.add(newTabFrame);                //KEY EVENT 2 : Add the TabFrame to the Array
		// ADDING tab to view
		tabPane.addTab(                                 //KEY EVENT 3 : Add Tab to view
														"File"
														+ Integer.toString(newFileCount), newTabFrame);
		tabPane.setSelectedIndex(tabPane.getTabCount()
								 - 1); //Helper, select new tab to be viewed
	...
	private void openMenuItemActionPerformed(){
		...
		if(status == JFileChooser.APPROVE_OPTION){
			...
			new OpenCommand(f, pane, this, tabPane.getTabCount()
										   - 1).execute(commandDistributor);
	...
	public void openFileWithoutFileChooser(File f){
		...
		new OpenCommand(f, newTabFrame.getTextPane(), this,
						tabPane.getTabCount() - 1).execute(commandDistributor);
	...
	private void insertTableActionPerformed(){
		...
		if((x.getRow() != 0) || (x.getCol()
								 != 0)){ //Making sure 'cancel' wasn't clicked
	...
	private void insertATagActionPerformed(){
		...
		if(href != ""){
			new InsertATagCommand(href, getActivePane()).execute(commandDistributor);
		}
	}
	...
	/**
	 * Action performed method when pressing the tabWidth button. Launches a
	 * dialog for the user to set the dat width.
	 */
	...
	private void addListeners(JTextArea newEditorPane){
		...
		newEditorPane.getDocument().addDocumentListener(commandDistributor.getFileManager().getFileAt(
	...
	/**
	 * @return The index of the active pane Will probably be removed
	 */
	...
	/**
	 * Static methods that will indent the tabs given the current count of the
	 * tab count.
	 *
